...
class ApiService {
  ...
  /// 供其他 service 共用同一個帶 auth interceptor 的 Dio instance
  Dio get dio => _dio;
  ...
  /// 後端 /api/auth/login 使用 JSON body（email + password），僅回傳 access_token。
  /// 需再呼叫 /api/auth/status 取得 uuid / name / email / phone_number。
  ...
  Future<LoginResponse> _mockLogin(LoginRequest req) async {
    ...
    return LoginResponse(
      ...
      uuid: 'u-mock',
      name: 'Mock User',
      email: req.email.isEmpty ? 'guest@example.com' : req.email,
      phoneNumber: '',
    );
  }
